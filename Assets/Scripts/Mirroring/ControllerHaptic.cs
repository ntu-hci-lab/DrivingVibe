    ...
    public void Controller_FeedbackReceived(object sender, Xbox360FeedbackReceivedEventArgs e)
    {
        if (gamepad != null)
        {
            UnityMainThread.wkr.AddJob(() =>
            {
                ...
                for (int i = 0; i < 16; i++)
                {
                    headband[i] = Mathf.CeilToInt(percentageIntensity * 100);
                }
...
[CustomEditor(typeof(ControllerHaptic))]
public class ControllerHapticBtn : Editor
{
    public override void OnInspectorGUI()
    {
        base.OnInspectorGUI();
        ControllerHaptic controllerHaptic = (ControllerHaptic)target;

        if (GUILayout.Button("GetPhysicalController"))
        {
            controllerHaptic.GetPhysicalController();
        }
        if (GUILayout.Button("DisconnectVirtualController"))
        {
            controllerHaptic.DisconnectVirtualController();
        }
    }
}